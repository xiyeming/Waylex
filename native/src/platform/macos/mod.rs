...
/// TrayIcon uses Rc<RefCell<...>> internally which is !Send, but on macOS
...
impl PlatformBackend for MacOsBackend {
    async fn register_hotkeys(&self, shortcuts: Vec<ShortcutBinding>) -> Result<(), HotkeyError> {
        ...
        tracing::info!("Registered {} macOS global hotkeys", registered.len());
        Ok(())
    }
    ...
    async fn screenshot(&self) -> Result<Vec<u8>, OcrError> {
        // macOS: use screencapture CLI with interactive area selection
        let temp_file = std::env::temp_dir().join("Waylex_screenshot.png");

        let output = std::process::Command::new("screencapture")
            .args(["-i", "-x", temp_file.to_str().unwrap()])
            .output()
        ...
        let data = std::fs::read(&temp_file).map_err(|e| OcrError::IoError(e))?;
    ...
    async fn init_tray(&self) -> Result<(), TrayError> {
        ...
        *tray = Some(SendTrayIcon(tray_icon));
        tracing::info!("macOS tray initialized");
        Ok(())
    }
